...
| R3.1 | `find_type_instances` can only search a PLC (`IInstanceSearchScope` is implemented by `PlcSoftware` alone in V21), so HMI faceplate types report 0 instances; the update check does list HMI instances | [R3–R5 acceptance](superpowers/acceptance/reports/2026-09-25-r3-r5-live.md) | Consider deriving HMI instances from `check_library_updates` or a screen-item scan |
