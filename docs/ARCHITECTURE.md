...
`library_read` uses the same DomainReads seam. `LibraryReadBuilder` discovers the project library
(object root `project`) and the global libraries already open in the Portal (object root `portal`,
paths starting at `GlobalLibraries`) over `IObjectNode`, lists types and master copies with
`GroupTreeLister`, and reads version associations through `IObjectNode.ReadObjectList`. The update
