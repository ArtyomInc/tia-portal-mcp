...
MCP server for Siemens TIA Portal V21. Exposes 16 tools in read-write mode and six read-only tools in read-only mode. Windows-only, requires TIA Portal V21 with Openness enabled.
